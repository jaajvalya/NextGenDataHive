 ...
 *
 * Requires the companion API: `python -m api` (port 5055).
...
(function (global) {
   ...
   * For file-upload mode, also stores the file under storage/uploads on the server.
  ...
  async function fetchRecentConnectors(limit) {
    ...
    if (res.status === 404) {
      ...
      throw new Error(
        "Connections API not found. Restart the connector API: python -m api"
      );
    }
    var bodyText = await res.text();
    var data = null;
