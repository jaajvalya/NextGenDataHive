...
(function (global) {
  ...
  ensureConnectorApi().then(function (ok) {
    if (!ok) {
      ...
      console.warn(
        isMac
          ? "[datahive] Local connector API unavailable. Start via scripts/Open DataHive UI.command " +
              "or run: .venv/bin/python api/connector_watchdog.py"
          : "[datahive] Local connector API unavailable. Start the UI via scripts/Open DataHive UI.bat " +
              "or run: pythonw api/connector_watchdog.py"
      );
      return;
    }
    sendPresence();
