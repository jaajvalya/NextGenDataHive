"""
Background helper for the UI — keeps the API running while main.html is open.

Listens on http://127.0.0.1:5056 (does not conflict with the API on 5055).
Run once at logon (hidden), e.g.:
    pythonw api/connector_watchdog.py

Or install for the current user:
    powershell -ExecutionPolicy Bypass -File scripts/install_ui_watchdog.ps1
"""
...
# Launched as a module so `api` and `core` resolve from the repository root.
_API_COMMAND = ("-m", "api")
_HOST = "127.0.0.1"
_API_PORT = 5055
_WATCHDOG_PORT = 5056
...
def _start_api() -> None:
    ...
    with _lock:
        ...
        _child = subprocess.Popen(
            [sys.executable, *_API_COMMAND],
            cwd=str(_REPO_ROOT),
            creationflags=flags,
        )
        _we_started_api = True
        log.info("Started the DataHive API (pid %s)", _child.pid)
...
def _stop_api() -> None:
    ...
    with _lock:
        ...
        try:
            ...
            proc.kill()
        log.info("Stopped the DataHive API")
...
def _reap_idle_tabs() -> None:
    """Drop stale tab presence. Do not stop the API — the UI expects :5055 to stay up."""
    global _tabs
    now = time.time()
    with _lock:
